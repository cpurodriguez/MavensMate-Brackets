...
define(function (require, exports, module) {
    ...
    function MMInterface() {
        ...
        this.nodeconn = new NodeConnection();
    ...
    MMInterface.prototype.call = function (params) {
        ...
        MMSettings.getSetting("mm_python_location").then(function(pythonLocation) {
            var python = pythonLocation;
            Utils.debug(python);
            MMSettings.getSetting("mm_debug_location").then(function(mm_location) {
                var mm = mm_location;
                Utils.debug(mm);
                var command = python+' '+mm+' -c BRACKETS -o '+operation;
                Utils.debug(command);
                Utils.debug(jsonPayload);
                Utils.debug(this.nodeconn);
                Utils.debug(this.nodeconn.domains);
                Utils.debug(this.nodeconn.domains.mmexec);
                this.nodeconn.domains.mmexec.runScript(command, jsonPayload, pId, {
    ...
    MMInterface.prototype.getNodeConnection = function () {
        return this.nodeconn;
    };
